...
int pidify_create(struct wangyonglin__config *config)
{
    ...
    if (len <= 0)
    {
        logify_printf(config, LOG_ERR, "Pid error (%s)", strerror(errno));
        return ERR_PID;
    }
    ...
    if (pid_fd < 0)
    {
        logify_printf(config, LOG_ERR, "unable to set pidfile '%s': %s", config->pid_file, strerror(errno));
        return ERR_PID;
    }
    if (lockf(pid_fd, F_TLOCK, 0) < 0)
    {
        logify_printf(config, LOG_ERR, "unable to lockf file: %s", strerror(errno));
        return ERR_PID;
    }
    ssize_t ret = write(pid_fd, val, (unsigned int)len);
    if (ret == -1)
    {
        logify_printf(config, LOG_ERR, "unable to write pidfile: %s", strerror(errno));
        close(pid_fd);
        return ERR_PID;
    }
    else if ((size_t)ret != len)
    {
        logify_printf(config, LOG_ERR, "unable to write pidfile: wrote"
...
int pidify_test(struct wangyonglin__config *config)
{
    if (access(config->pid_file, F_OK) == 0)
    {
        ...
        if (pf == NULL)
        {
            logify_printf(config, LOG_ERR, "pid file '%s' exists and can not be read. Aborting!",
                        config->pid_file);
            return ERR_PID;
        }
        ...
        {
            fclose(pf);
            logify_printf(config, LOG_ERR, "pid file '%s' exists. Is program already running? Aborting!",
                        config->pid_file);
            return ERR_PID;
        }
